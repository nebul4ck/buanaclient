...
class buanaClient(object):
	...
	def start_program(self, command, arg_cmd, opt, client_host, branch):
		...
		if re.findall(r'not found', str(self.buanarepoServerUrl)):
			...
			if command == 'deploy':
				...
				if command == 'sync':
					...
					git_data = json.dumps(json_data)

				else:
					git_data = self.test_git(client_host,\
											arg_cmd, branch, opt)
	...
	def deployer(self, command, arg_cmd, deploy_mode, url, basic_auth,\
	            buanaCA):
	...
	def buanaLocalDeploy(self, data, basic_auth, buanaCA, buanarepoUrl):
		...
		if can_do_it is not True:
			...
			for host, services in json_data.iteritems():
				...
				p = Process(name='orchestration-host',\
	...
	def buanaRemoteDeploy(self, data, basic_auth, buanaCA, buanarepoUrl, command):
		...
		if command == 'deploy':
			...
			l.debug("The command is: %s" % command)
		...
		if can_do_it is not True:
			...
			try:
				...
				send_post = requests.post(runner_command,\
											verify=buanaCA, headers=headers,\
